...
/*
 ...
 （11）storyboard生成ViewController并添加按钮+storyboard在第一个ViewController中添加导航栏+使用presentViewController跳转到另一个xib生成的ViewController；-->跳转正常，但是第二个界面不会出现导航栏，该方法不可行，同（6）；
 （12）storyboard生成ViewController并添加按钮+storyboard在第一个ViewController中添加导航栏+使用pushViewController跳转到另一个xib生成的ViewController；-->正常，可以进行跳转，并可以使用导航栏自带的返回按钮返回上一个页面；同（5）；
